...
public:
  GridGraphics(Location start, Location goal) {
...
//    }
  ...
  void draw(sf::RenderWindow &window, const std::vector<Location> &nodes, bool path_found) {
    sf::Vertex line[2];
    ...
    for(int i = (int)nodes.size() - 1; i > 0; i--) {
      Location par = nodes[i] ;
      line[0] = sf::Vertex(sf::Vector2f(par.x, par.y));
      line[1] = sf::Vertex(sf::Vector2f(nodes[i - 1].x, nodes[i - 1].y));
